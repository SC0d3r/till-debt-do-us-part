 ...
 *   5. ORIGIN GUARANTEE (design-critic Major 1): the origin chunk (0,0) is
 *      pinned to grass AFTER every pass (raw Voronoi pin + final pin in
 *      chunkBiomeAt), and the origin TILE (0,0) is pinned solid in
 *      isTileSolid — the player always spawns on solid grass at (0,0).
 ...
 * band, rare per-tile roll). The origin tile (0,0) is ALWAYS solid — the
 * player spawn guarantee, pinned after the island mask. Pure + deterministic.
 */
...
/** Biome for a global tile coordinate: the biome of its chunk for solid
 *  tiles, 'void' for void tiles (fix round 1: void tiles have no biome — the
 *  world manager's public API documents this). Deterministic. */
