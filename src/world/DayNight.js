 ...
 *
 * No sun/moon meshes — light only (the isometric camera never sees the sky).
 *
...
export class DayNight {
  ...
  /**
   * Phase + day factor for a timeOfDay (0..1439).
   *
  ...
  phaseAt(timeOfDay) {
    ...
    if (t < DAY_START) return { phase: 'dawn', dayFactor: (t - DAWN_START) / (DAY_START - DAWN_START) }
    if (t < DAY_END) return { phase: 'day', dayFactor: 1 }
    if (t < DUSK_END) return { phase: 'dusk', dayFactor: 1 - (t - DAY_END) / (DUSK_END - DAY_END) }
    return { phase: 'night', dayFactor: 0 }
  }

  /**
   * Writes the lerped light values into the world's lighting rig (scratch
   * colors — no allocation). Also returns the day factor so the world can
   ...
   */
  apply(rig, timeOfDay) {
    const { dayFactor } = this.phaseAt(timeOfDay)
