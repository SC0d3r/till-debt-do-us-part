...
/** Spawn-patch radius (fix round 2): every tile with max(|x|,|y|) <=
 *  SPAWN_PIN_RADIUS around the origin is ALWAYS solid — a 5×5 data-space
 *  patch (25 tiles) so the player never boots to a 1-tile island in the
 *  void. QA hook: tests assert the origin-chunk tile floor for many seeds. */
export const SPAWN_PIN_RADIUS: number
