...
/** Prop instance-color night floor: props are UNLIT (baked MeshBasicMaterial
 *  — reference A), so their dims must carry the night dip themselves. Per
 *  prop: `nightFloor + (1 − nightFloor)·dayFactor` — matches the rig's night
 *  ambient so props read as silhouettes, never glow (completion round). */
const PROP_NIGHT_FLOOR = 0.06
/** Mid-band dims refresh step (tiles): when the natural day/night FOV tween
 *  moves ≥ this amount, _applyFovDims() runs again — the dims track the band
 *  INSIDE a chunk-radius interval, not just at crossings (completion round).
 *  0.5 ≈ 6 refreshes across the 3-tile fade band; the max band-lag after a
 *  refresh is 1.5·0.5/3 = 0.25 in dim (smoothstep derivative ≤ 1.5). */
const FOV_DIMS_STEP = 0.5
...
export class WorldManager {
  ...
  constructor(scene, camera, { seed = 1337, isActive }) {
    ...
    // Player cube (plain BoxGeometry — no model yet). Lambert + a small
    // emissive (completion round): inside the night pool the cube silhouette
    // stays readable without hot clipping — the emissive is ~8% of the day
    // face, so the day look is nearly unchanged.
    this.playerMesh = new THREE.Mesh(
      new THREE.BoxGeometry(0.5, 0.5, 0.5),
      new THREE.MeshLambertMaterial({ color: 0xff8c42, emissive: 0xffb066, emissiveIntensity: 0.14 }),
    )
    ...
    // Pool light rides high + forward (completion round: was (0, 1.2, 0) —
    // too low to light the near tiles' faces and the cube's front) so the
    // pool reaches the tiles ahead of the player.
    this.playerLight.position.set(0, 1.7, 0.4)
    ...
    /** FOV radius the current dims were computed at (completion round: the
     *  dims must track the mid-band tween too — _applyDayNight refreshes
     *  when |fovRadius − _lastDimsFov| ≥ FOV_DIMS_STEP). */
    this._lastDimsFov = this.fovRadius
    /** Last day factor written to the rig (completion round: props are
     *  unlit, so _applyFovDimsToChunk encodes the night dip into their
     *  instance colors: PROP_NIGHT_FLOOR + (1 − PROP_NIGHT_FLOOR)·dayFactor). */
    this._dayFactor = 1
  ...
  _applyDayNight() {
    ...
    if (this._fovOverride === null) {
      ...
      // The >0.001 gate alone skips the final tween step (dusk: fov
      // 5.0003 → 5.0, delta 0.0003 < 0.001), leaving fovRadius stuck at
      // 5.0003 → loadR 8.0003 → chunk radius stays 2 → the natural dawn/
      // dusk boundary crossing NEVER fires and retained chunks keep stale
      // dims (found by the corrective-round-3 natural-tween probe). Snap
      // to the exact terminal values (dayFactor is exactly 0/1 in the
      // settled phases, so fov === NIGHT_FOV/DAY_FOV is exact) — the
      // crossing always fires and _updateRadii refreshes the dims.
      if (fov !== this.fovRadius && (Math.abs(fov - this.fovRadius) > 0.001 || fov === NIGHT_FOV || fov === DAY_FOV)) {
        this.fovRadius = fov
        this._updateRadii()
      }
      // Mid-band dims tracking (completion round): the dims previously only
      // refreshed when the load radius crossed a chunk boundary (or on
      // player-tile change / harness setters), so a natural dusk/dawn tween
      // left the fog band lagging the FOV by most of the band. Refresh when
      // the tween moves ≥ FOV_DIMS_STEP (0.5) — ~6× per 3-tile band. 
      // _applyFovDims stamps _lastDimsFov, so the chunk-crossing refresh in
      // _updateRadii and this check can never double-apply.
      // TEMP-REVERTED FOR PRE-FIX PROOF (CI run in progress): if (Math.abs(this.fovRadius - this._lastDimsFov) >= FOV_DIMS_STEP) this._applyFovDims()
    }
  }
  ...
  _buildChunk(key) {
    ...
    if (data.tiles.length > 0) {
      ...
      // chunk (corrective round 3). With the corrected center the farthest
      // tile center is ~3.5 from it: every diamond corner sits ±3.5 in x or
      // z (tile (7,7) at world z = 7 vs center z = 3.5) and NO tile is 3.5
      // away in BOTH axes, so the old "~4.95" figure double-counted the
      // diagonal. The sphere is the raycast early-out (fix round 1: cheap
      // distance check before any mesh raycast).
  ...
  _buildPropMeshes(props, parent) {
    ...
    for (const [name, list] of byName) {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        // Night factor at BUILD time (completion round): a chunk spawned
        // during the night starts at the current night dim — no one-frame
        // bright flash before the FOV pass runs. Same formula as the FOV
        // pass, applied to a full-bright (1.0) base.
        mesh.setColorAt(i, this._scratchColor.setScalar(PROP_NIGHT_FLOOR + (1 - PROP_NIGHT_FLOOR) * this._dayFactor))
      }
  ...
  _applyFovDims() {
    // Stamps the FOV these dims were computed at — the mid-band refresh in
    // _applyDayNight keys off this, so every caller (tween, chunk-radius
    // crossing, teleport, harness setters) stays consistent.
    this._lastDimsFov = this.fovRadius
  ...
  _applyFovDimsToChunk(chunk) {
    ...
    for (const pm of chunk.propMeshes) {
      // Props are unlit (baked MeshBasicMaterial — reference A) so their
      // instance color must carry BOTH the FOV band fog AND the night dip
      // (completion round): factor = fog · (0.06 + 0.94·dayFactor) — props
      // darken with the rig instead of glowing at night.
      const nightDim = PROP_NIGHT_FLOOR + (1 - PROP_NIGHT_FLOOR) * this._dayFactor
      for (let i = 0; i < pm.props.length; i++) {
        const p = pm.props[i]
        this._scratchColor.setScalar(this._fogFactor(p.x, p.y) * nightDim)
