 ...
 *  ((cx−cy)·4, (cy+cx)·4); radius 5.2 covers the corners + tile/prop height. */
const CHUNK_BOUNDS_RADIUS = 5.2
/** Hover ring around the hovered tile (fix round 2): a thin halo just above
 *  the tile cap, slightly outside the ±0.5 diamond. */
const HOVER_RING = true
/** Reveal radius in data tiles (fix round 2): every tile inside a circle of
 *  this radius around the hovered tile is forced to full brightness while
 *  hovered — the FOV fog band lights up around the cursor. 0 disables. */
...
export class WorldManager {
   ...
   *  streaming queue when the radius grows) — refreshes streaming. The
  ...
  _buildChunk(key) {
    ...
    if (data.tiles.length > 0) {
      ...
      composer = new TileMapComposer({
        ...
        // Round 2: hover halo + fog-band reveal around the cursor.
        hoverRing: HOVER_RING,
        revealRadius: REVEAL_RADIUS,
      })
      propMeshes = this._buildPropMeshes(data.props, group)
      this.scene.add(group)
      // Chunk diamond center ((cx−cy)·4, ·, (cy+cx)·4), half-extent 3.5 in
      // world x/z plus tile/prop height — bounding sphere for the raycast
      // early-out (fix round 1: cheap distance check before any mesh raycast).
      boundsSphere = new THREE.Sphere(
        new THREE.Vector3((cx - cy) * 4, 0.3, (cx + cy) * 4),
        CHUNK_BOUNDS_RADIUS,
      )
    }
    const chunk = {
