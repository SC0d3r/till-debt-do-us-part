...
export function initDevHarness(game: unknown, graph: DevHarnessGraph): void {
  ...
  async function showcaseTileMap(data?: TileMapRecord[], opts?: { outline?: TileMapOutlineOptions }): Promise<void> {
    ...
    // previewAsset (32° elevation, camera from the south/+z). B2: pulled in
    // ~30% from (0, 8.2, 16.8) and lookAt aimed at the map's vertical middle
    // so the map fills ≥75% of the frame height instead of ~48%.
    cam.position.set(0, 4.5, 9.8)
    cam.lookAt(0, 0.15, 3.6)
  ...
  async function propsShowcase(): Promise<void> {
    ...
    // (world span ~5.5 x ~5.5 units). B2: pulled in ~30% from (0.5, 6.2, 12.2)
    // and lookAt aimed at the diorama's vertical middle so it fills ≥75% of
    // the frame height.
    cam.position.set(0.5, 3.4, 7.6)
    cam.lookAt(0.25, 0.15, 2.6)
