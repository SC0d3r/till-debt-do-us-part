...
export function initDevHarness(game: unknown, graph: DevHarnessGraph): void {
  ...
  const fixtureSetups: Record<string, () => void | Promise<void>> = {
    ...
    // Round-2 spawn QA scene: noon, debug overlay off, the red origin marker
    // at the mesh center of tile (8,8). Seed 11 is the first candidate with
    // tile (8,8) SOLID grass (verified by tests/qa-spawn-round2.mjs — the
    // marker must not float over void); the 5×5 spawn patch
    // (max(|x|,|y|) <= 2) is solid grass for every seed.
