 ...
 *  position + color (white default) + computed flat per-face normals. */
...
export function buildPost({ r = 0.02, rTop, h, y0 = 0, sides = 6, tones, bands = 2, aoHeight = 0.28 }) {
  ...
  for (let t = 0; t < pos.length / 9; t++) {
    const a = t * 9
    const ay = (pos[a + 1] + pos[a + 4] + pos[a + 7]) / 3
    const hRel = (ay - y0) / h
...
/**
 * Dark edge ribbons along every crease of a geometry (dihedral angle above
 * `minDihedral`), offset outward so they never z-fight. Merged into the body
 * geometry — same material, vertex colors encode outline vs body. This is the
 * tinted non-black silhouette outline of reference B, InstancedMesh-safe.
 *
 * @param {THREE.BufferGeometry} geo
 * @param {object} opts
 * @param {number|string|number[]} opts.color - outline tone
 * @param {number} [opts.width=0.011] - ribbon width
 * @param {number} [opts.minDihedral=0.12] - crease threshold (radians)
 * @returns {THREE.BufferGeometry}
 */
export function buildRibbons(geo, { color, width = 0.011, minDihedral = 0.12 }) {
  ...
  for (const [key, faces] of edges) {
    ...
    let d
    if (faces.length >= 2) {
      const n1 = tris[faces[0]].n
      const n2 = tris[faces[1]].n
    ...
    const p1 = [pos[ia * 3], pos[ia * 3 + 1], pos[ia * 3 + 2]]
    const p2 = [pos[ib * 3], pos[ib * 3 + 1], pos[ib * 3 + 2]]
