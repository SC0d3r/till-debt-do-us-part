...
export function shadeFaces(geo, { light, mid, dark, outline }, { aoHeight = 0.22, aoAmount = 0.55, hi, lo } = {}) {
  ...
  for (let t = 0; t < pos.length / 9; t++) {
    ...
    for (const i of [a, b, c]) {
      // B3 (re-review): shadeFaces previously wrote raw full-hex tones,
      // bypassing PROP_BRIGHTNESS — props rendered ~1.4x brighter than tiles.
      // Scale the FINAL tone (after AO blend) so every shaded face matches the
      // baked tile brightness exactly like finishGeo/buildBox/buildBlade do.
      col[i] = r * PROP_BRIGHTNESS
      col[i + 1] = g * PROP_BRIGHTNESS
      col[i + 2] = bl * PROP_BRIGHTNESS
    }
  }
  return geo
}
...
export function makeCelMaterial({ side = THREE.FrontSide } = {}) {
  return new THREE.MeshBasicMaterial({ vertexColors: true, side })
}
