...
function getGeometry() {
  if (!geometry) {
    const parts = []
    // Fanned blades from a common base (F1): yaws spread to ±1.1 rad with
    // alternating leans so the tuft reads as separate blades in both the
    // close preview and the wide diorama; blades ~1.5x wider and heights
    // 0.16-0.36 so the tuft reads as TALL grass, not one thin needle.
    const blades = [
      { h: 0.36, w: 0.055, yaw: -0.75, lean: 0.3 },
      { h: 0.28, w: 0.048, yaw: 0.85, lean: 0.16 },
      { h: 0.22, w: 0.042, yaw: -1.05, lean: 0.26 },
      { h: 0.32, w: 0.052, yaw: 0.1, lean: 0.34 },
      { h: 0.18, w: 0.038, yaw: 1.1, lean: 0.1 },
      { h: 0.25, w: 0.045, yaw: -0.35, lean: 0.22 },
    ]
