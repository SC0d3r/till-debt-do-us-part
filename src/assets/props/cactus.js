...
function getGeometry() {
  if (!geometry) {
    ...
    for (const az of armAz) {
      ...
      const elbow = buildPost({ r: ARM_R, rTop: ARM_R, h: 0.05, sides: 6, tones: C, bands: 1, aoHeight: 0.2 })
      elbow.applyMatrix4(new THREE.Matrix4().makeRotationZ(-Math.PI / 2))
      elbow.applyMatrix4(new THREE.Matrix4().makeRotationY(az))
      elbow.translate(sa * 0.052, ARM_Y, ca * 0.052)
      body.push(elbow)
      // Rise: tapered column up from the elbow's outer end.
      const rise = buildPost({ r: ARM_R, rTop: ARM_R * 0.78, h: ARM_H, y0: 0, sides: 6, tones: C, bands: 2, aoHeight: 0.25 })
      rise.translate(sa * 0.078, ARM_Y + 0.014, ca * 0.078)
      body.push(rise)
      // Arm tip cap.
      const tip = buildIcosa({ r: ARM_R * 0.78, detail: 1, seed: 84 + Math.round(az * 10), jitter: 0.06, squash: 0.5 })
      tip.translate(sa * 0.078, ARM_Y + 0.014 + ARM_H, ca * 0.078)
      shadeFaces(tip, C, { aoAmount: 0.3, aoHeight: 0.3 })
      body.push(tip)
      // Spines on the arm rise.
      for (let i = 0; i < 3; i++) {
        const sy = ARM_Y + 0.02 + i * 0.032
        extras.push(buildSpineDot({ r: ARM_R * (1 - (0.22 * (i + 0.5)) / 3), y: sy, theta: az + 0.15, size: 0.004, color: SPINE }))
      }
    }
    ...
    for (let i = 0; i < spineThetas.length; i++) {
      ...
      extras.push(buildSpineDot({ r: rAtY, y: sy, theta: spineThetas[i], size: 0.0045, color: SPINE }))
    }
