...
const PETAL = { light: 0xffffff, mid: 0xf0f2f5, dark: 0xd4d7db, outline: 0x2a4a1c }
const CORE = { light: 0xffe08a, mid: 0xffd54f, dark: 0xe0a92e, outline: 0x8a6a10 }
const STEM = { light: 0x4c8730, mid: 0x4c8730, dark: 0x2d5a27, outline: 0x2d5a27 }
...
function getGeometry() {
  if (!geometry) {
    const parts = []
    // Loose triangular scatter; per-stem variety (yaw/lean/scale).
    const flowers = [
      { off: [-0.045, 0.028], h: 0.115, yaw: 0.4, lean: 0.09, scale: 1.0, seed: 71 },
      { off: [0.05, 0.002], h: 0.13, yaw: -0.35, lean: 0.05, scale: 0.9, seed: 72 },
      { off: [0.004, 0.058], h: 0.1, yaw: 1.15, lean: 0.13, scale: 1.08, seed: 73 },
    ]
    for (const fl of flowers) {
      const base = new THREE.Matrix4().makeTranslation(fl.off[0], 0, fl.off[2])
      ...
      // Stem: thin tapered hex post, slightly buried.
      ...
      // Petals: flattened dome on the stem tip.
      const petal = buildIcosa({ r: 0.03 * s, detail: 1, seed: fl.seed, jitter: 0.1, squash: 0.3 })
      petal.translate(0, fl.h + 0.004, 0)
      shadeFaces(petal, PETAL, { aoAmount: 0.15, aoHeight: 0.1 })
      ...
      // Tiny dark contact dot under the flower.
      const dot = buildContactRing({ r: 0.016 * s, color: PETAL.outline, thickness: 0.012, height: 0.006, y: 0.001 })
      dot.applyMatrix4(new THREE.Matrix4().makeTranslation(fl.off[0], 0, fl.off[2]))
      parts.push(dot)
    }
