...
function getGeometry() {
  if (!geometry) {
    ...
    parts.push(body)
    // Dark edge ribbons on the SHARP creases only (F3: minDihedral 0.55 keeps
    // the silhouette outline but drops the wireframe-y jitter facet edges).
    parts.push(buildRibbons(body, { color: R.outline, width: 0.011, minDihedral: 0.55 }))
    ...
    parts.push(buildContactRing({ r: 0.15, color: R.outline }))
