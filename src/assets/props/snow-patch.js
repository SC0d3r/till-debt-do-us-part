...
// ─── Palette (snow ramp) ───
// F9: shadow tone deepened to ≈0x9fb2c9 so drifts separate from the snow
// tile; the base AO blends toward the pale-blue outline (contact tone).
const SNOW = { light: 0xffffff, mid: 0xdbe6f0, dark: 0x9fb2c9, outline: 0x7e93ad }
...
function getGeometry() {
  if (!geometry) {
    const parts = []
    const drifts = [
      { r: 0.055, jitter: 0.24, squash: 0.5, seed: 877, off: [-0.02, 0.02] },
      { r: 0.05, jitter: 0.24, squash: 0.5, seed: 878, off: [0.05, 0.01] },
      { r: 0.042, jitter: 0.26, squash: 0.52, seed: 879, off: [-0.05, -0.025] },
      { r: 0.035, jitter: 0.24, squash: 0.5, seed: 880, off: [0.01, -0.055] },
    ]
    for (const dr of drifts) {
      const g = buildIcosa({ r: dr.r, detail: 1, seed: dr.seed, jitter: dr.jitter, squash: dr.squash })
      sitOnGround(g)
      // hi=0.7 reserves the light tone for the very top facets: on a squashed
      // pale mound the default buckets collapse into one light tone and the
      // drift reads flat against the snow tile. aoAmount 0.65 + tall AO band
      // paints a pale-blue contact tone around the drift bases (F9).
      shadeFaces(g, SNOW, { aoAmount: 0.65, aoHeight: 0.35, hi: 0.7, lo: 0.05 })
