...
function getGeometry() {
  if (!geometry) {
    const parts = []
    const pebbles = [
      { r: 0.034, jitter: 0.5, squash: 0.62, seed: 61, off: [-0.03, 0.035], tan: false },
      { r: 0.028, jitter: 0.55, squash: 0.66, seed: 62, off: [0.062, 0.052], tan: true },
      { r: 0.025, jitter: 0.5, squash: 0.62, seed: 63, off: [-0.082, -0.04], tan: false },
      { r: 0.032, jitter: 0.55, squash: 0.64, seed: 64, off: [0.03, -0.078], tan: false },
      { r: 0.023, jitter: 0.6, squash: 0.66, seed: 65, off: [0.085, -0.058], tan: true },
      { r: 0.021, jitter: 0.5, squash: 0.62, seed: 66, off: [-0.062, 0.08], tan: false },
    ]
    for (const pb of pebbles) {
      const g = buildIcosa({ r: pb.r, detail: 0, seed: pb.seed, jitter: pb.jitter, squash: pb.squash, yaw: pb.seed })
      sitOnGround(g)
      // Tight light bucket (F6 + round-3): keeps a visible light top facet on
      // each pebble and pushes the rest into mid/dark so the scatter reads as
      // separate 3D stones, not one merged pile.
