...
function getGeometry() {
  if (!geometry) {
    // F4: jitter lowered so the base reach (r·(1+jitter) ≈ 0.073) fits a
    // footprint-scale ring (~0.08) — the old 0.105 ring dominated the pebble.
    const body = buildIcosa({ r: 0.062, detail: 0, seed: 41, jitter: 0.18, squash: 0.7, yaw: 0.7 })
    sitOnGround(body)
    // hi=0.7 keeps the light tone on the very top facets only, so the pebble
    // shows a proper light-cap/mid-body/dark-side ramp instead of reading flat.
    shadeFaces(body, S, { aoAmount: 0.55, hi: 0.7, lo: 0.05 })
    const parts = [body]
    parts.push(buildRibbons(body, { color: S.outline, width: 0.01, minDihedral: 0.55 }))
    parts.push(buildContactRing({ r: 0.08, color: S.outline, thickness: 0.018 }))
