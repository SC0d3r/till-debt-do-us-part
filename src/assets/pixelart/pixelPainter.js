 ...
 *   (e.g. a mask restricting noise to one half of the tile); noise never
 *   escapes it
 ...
 * @param {number} [opts.skipChance=0.15] - fraction of edge steps skipped
 * @param {number} [opts.thickness=3] - max px inside the rim (2-3 texel strokes)
 */
export function paintJaggedOutline(ctx, opts = {}) {
  ...
  const half = size / 2
  const skipChance = opts.skipChance ?? 0.15
  ...
  for (const [ai, bi] of edges) {
    ...
    for (let k = 0; k <= len; k++) {
      ...
      const off = 1 + Math.floor(rng.next() * (thickness - 1)) // 1..(thickness-1) px inside
      const px = Math.round(ex + ux * off) + jx
      const py = Math.round(ey + uy * off) + jy
      if (px < 0 || py < 0 || px >= size || py >= size) continue
      setPixel(ctx, px, py, color)
      // occasional thickening (2-3px run along the edge)
      if (rng.next() < 0.3) {
        const qx = px + Math.round(tx)
        const qy = py + Math.round(ty)
        if (qx >= 0 && qy >= 0 && qx < size && qy < size) setPixel(ctx, qx, qy, color)
        if (rng.next() < 0.5) {
          const rx = qx + Math.round(tx)
          const ry = qy + Math.round(ty)
          if (rx >= 0 && ry >= 0 && rx < size && ry < size) setPixel(ctx, rx, ry, color)
        }
      }
    }
  }
