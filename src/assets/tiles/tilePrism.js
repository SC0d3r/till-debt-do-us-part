...
const WHITE_UV = 0.5 / TILE_TEXTURE_SIZE
const WHITE_UV_V = 1 - WHITE_UV

const WHITE = [1, 1, 1]

/**
 * Normalized 0-1 vertex color for a hex color. THREE stores hex colors in the
 * linear working space when color management is enabled; vertex colors are
 * consumed as-is by the shader, so use the color's own components (NOT the
 * sRGB bytes the canvas writers use — those are 0-255 and would clamp to
 * white as vertex colors).
 */
...
export function buildTilePrism(topY, baseY, sideColors) {
  ...
  for (let i = 0; i < 4; i++) {
    const [a, b] = edges[i]
    const [br, bg, bb] = hexToColor(sideColors[i])
