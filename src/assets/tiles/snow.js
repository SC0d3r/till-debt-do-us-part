...
/** Baked-in texture outline (cool pale blue — never black; F1: darkened so
 *  snow carries the same style-feature outline as the other five biomes). */
export const SNOW_OUTLINE = 0x6e849e
...
function makeSnowTopTexture(seed, opts = {}) {
  return makePixelTexture(SIZE, (ctx, w, h) => {
    ...
    paintJaggedOutline(ctx, { rng, color: SNOW_OUTLINE, skipChance: 0.1 })
  })
}
...
export const createTile = (variant) => createSnowTile(variant)