...
- [shipped] (Assets & Art, S) tile-kit: decorative props base set — crystals (color variants), rocks/boulders (small+large), low-poly tree/bush, grass tufts, flowers; each a small reusable factory, heavily instanced, composable onto tile top faces by the composer. (Slice B section 4 socket system applies.) SHIPPED 2026-08-08 as part of Slice B: 15 props (flower, rock, bush, tall-grass, cactus, small-stone, big-stone, pebble-cluster, torch, lantern, gravel-patch, dry-shrub, bush-snow, snow-patch, lava-rock) via shared `propBase.js` builders + `props-showcase` diorama; socket metadata on each; 29 fixtures total. Followups queued below.
...
## Follow-ups from Slice B review (2026-08-08)

- [idea] (UI/UX & Accessibility, S) selection ring ownership pin — render own
  ring geometry for neighbor-owned seams or force owner re-resolution
  (ui-critic + design-critic: selection ring vs dropped-seam rule unpinned).
- [idea] (UI/UX & Accessibility, XS) hover cursor affordance — pointer cursor
  on hoverable tiles/props; DOM-overlay raycast gate (ui-critic).
- [idea] (Tech & Performance, S) outline frame cache key — must include
  (topY, baseY) once families differ in height (performance-critic Major
  latent; currently all families share one height so it's dormant).
- [idea] (Tech & Performance, M) prop instancing — props are per-instance
  meshes today; convert to InstancedMesh when a map carries many of one
  type (performance-critic).
- [idea] (Tech & Performance, XS) dead-code sweep — remove unused
  `makeTileCanvasTexture` export (zero callers) and the
  `kind: 'plain'|'decorated'|'edge'` union in tile d.ts (performance-critic).
- [idea] (Assets & Art, XS) prop polish nits — lava-rock contact ring gap,
  flower ring thickness, bush-snow ring contrast, pebble-cluster flatness at
  wide shots, ribbon epsilon (asset-critic followups).
- [idea] (Assets & Art, S) organic-boundary demo — a map with curved biome
  boundaries (staircased) to prove the plain-only kit reads at real
  boundaries before Slice C (design-critic).
- [idea] (Assets & Art, S) lava-plain-b variant — second lava texture
  variant for parity with other families (design-critic).
