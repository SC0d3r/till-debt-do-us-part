...
- **`getState()`** — plain serializable snapshot of the current state:
  `{ ready, started, fastMode: { enabled, renderEvery, dtScale, ticks } }`.
  No THREE objects, no DOM.
- **`gotoFixture(name)`** — resets everything, then applies a named fixture's
  setup (see below). The reset step tears down any active preview and wipes the
  `till_debt_save` / `till_debt_farm` localStorage keys — so fixtures never
...
- **`setFastMode(enabled, renderEvery?, dtScale?)`** — runtime toggle for fast
...
of any active preview) and by wiping the two localStorage save keys
