...
const scan = () => {
  ...
  return { solid, seed: w.seed, iso: countIso(solid) }
}
...
for (const seed of [1337, 777, 4242]) {
  ...
  out.rows.push({ seed, seedA: a.seed, seedB: b.seed, isoA: a.iso, isoB: b.iso, isoC: c.iso, same: a.iso.n === b.iso.n && b.iso.n === c.iso.n })
}
// Pure scans for seed 777 without regenerating (determinism check)
out.pureScans.push(await page.evaluate(scan))
out.pureScans.push(await page.evaluate(scan))
