...
section('R5. Natural dusk tween refreshes retained chunks\' dims (FOV band fix)')
{
  ...
  // radius 2 → 1 crossing happens ONLY at t >= 1200, where the dayFactor lerp
  // returns exactly 0 → fov == 5.0 EXACTLY (IEEE: 5 + 4·0 = 5), and fov stays
  // 5.0 through the whole night — so `fov <= 5.0` can only fire AFTER the
  // crossing (any pre-crossing threshold like 5.001 would race the dims
  // refresh by up to ~2ms). Fast mode advances ~12.5 in-game min/s, so
  // dusk's last ~100 min take ~8s; timeout is generous.
  const crossed = await (async () => {
    const deadline = Date.now() + 30000
    while (Date.now() < deadline) {
      const r = await evl(page, () => window.__debug.world.fovRadius)
      if (r.ok && r.value <= 5.0) return true
      await new Promise(res => setTimeout(res, 100))
    }
    return false
  })()
