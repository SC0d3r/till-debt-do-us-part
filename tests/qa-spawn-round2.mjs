...
section('R5. Natural dusk tween refreshes retained chunks\' dims (FOV band fix)')
{
  ...
  await waitChunksDrained(page)
  // Poll until the natural tween has crossed the dusk boundary (fov <= 5.05
  // ↔ t >= 1200). Fast mode advances ~12.5 in-game min/s, so dusk's last
  // ~100 min take ~8s; timeout is generous for slow CI runners.
  const crossed = await (async () => {
    const deadline = Date.now() + 30000
    while (Date.now() < deadline) {
      const r = await evl(page, () => window.__debug.world.fovRadius)
      if (r.ok && r.value <= 5.05) return true
      await new Promise(res => setTimeout(res, 100))
    }
    return false
  })()
