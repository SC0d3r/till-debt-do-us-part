...
section('R2. QA spawn scene: origin marker at the MESH CENTER of tile (8,8)')
{
  ...
  // the marker (it is a fixed debug object). The found seed is reported so
  // the fixture can be pinned to it.
  let chosen = null
  const candidates = [1337, 4242, 1, 2, 3, 42, 99, 777, 2024, 7, 11, 13, 17, 23, 31, 47, 53, 61, 89, 101, 500, 999, 1234, 5678, 31337]
  ...
  test('R2e tile (8,8) is solid grass in the QA scene (marker does not float over void)',
    ...
    r.ok ? `variant=${v?.tileVariant} foundSeed=${chosen?.seed} (${candidates.length} candidates tried)` : r.error)
...
section('R3. Fog dim → cursor reveal circle → hover ring → restore')
{
  ...
  const hover = await evl(page, () => {
    ...
    return {
      ...
      // (0,3): 3 tiles from the player (fog-dimmed) and 4.24 from the hovered
      // tile — OUTSIDE the reveal circle (2.5), so it must stay dim. (Not
      // (0,0): that is the tile under the player, dist 0 → full bright by
      // fog itself.)
      dimFar: w.tileDimAt(0, 3),
