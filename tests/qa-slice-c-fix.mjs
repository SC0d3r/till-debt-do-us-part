...
section('S3. Stepping stones: sparse isolated connectors (3 seeds)')
{
  ...
  const r = await evl(page, () => {
    ...
    for (const seed of [1337, 777, 4242]) {
      ...
      isoCounts.push(iso)
      // Determinism: re-scan a second time, must be identical.
      const solid2 = new Set()
      for (let cy = -4; cy <= 4; cy++) for (let cx = -4; cx <= 4; cx++) {
        for (const t of w.chunkData(cx, cy).tiles) solid2.add(`${t.x},${t.y}`)
      }
      let iso2 = 0
      for (const key of solid2) {
        const [x, y] = key.split(',').map(Number)
        if (!solid2.has(`${x + 1},${y}`) && !solid2.has(`${x - 1},${y}`) && !solid2.has(`${x},${y + 1}`) && !solid2.has(`${x},${y - 1}`)) iso2++
      }
      isoCounts.push(iso2)
    }
    return isoCounts
