...
// window blur (no InputManager blur handling), and the night→dawn tween while
// the player is moving (R5 at speed).
//
// Findings this battery has already confirmed (kept as regression probes):
//   W1  InputManager has NO window blur / visibilitychange key-reset: a WASD
//       key held across a tab-out keeps walking when focus returns (the keyup
//       is swallowed by the OS). Minor — no crash, but the cube walks off the
//       island unsupervised.
...
section('B. Extreme inputs: seeds, fov, clock boundaries')
{
  ...
  const fback = await evl(page, () => window.__debug.resetFovOverride !== undefined
    ? (window.__debug.resetFovOverride(), true) : false)
  test('B4b FOV override resets back to the day/night tween', fback.ok && fback.value === true, fback.ok ? '' : fback.error)

  // Clock boundaries + midnight wrap.
  const b0 = await evl(page, () => window.__debug.setState({ timeOfDay: 0 }))
  const s0 = await evl(page, () => window.__debug.getState())
  test('B5 timeOfDay 0 (midnight boundary): phase night, fov 5, no errors',
    b0.ok && s0.ok && s0.value.timeOfDay === 0 && s0.value.dayNightPhase === 'night' && Math.abs(s0.value.fovRadius - 5) < 0.001,
    s0.ok ? `tod=${s0.value.timeOfDay} fov=${s0.value.fovRadius} phase=${s0.value.dayNightPhase}` : s0.error)
  const b1439 = await evl(page, () => window.__debug.setState({ timeOfDay: 1439 }))
  const s1439 = await evl(page, () => window.__debug.getState())
  test('B6 timeOfDay 1439 (end-of-day boundary): stays 1439, phase night, no wrap glitch',
    b1439.ok && s1439.ok && s1439.value.timeOfDay === 1439 && s1439.value.dayNightPhase === 'night',
    s1439.ok ? `tod=${s1439.value.timeOfDay} fov=${s1439.value.fovRadius}` : s1439.error)
  ...
  const e359 = await evl(page, () => window.__debug.setState({ timeOfDay: 359 }))
  const s359 = await evl(page, () => window.__debug.getState())
  test('B9 timeOfDay 359 (last dawn minute): fov between 5 and 9, phase dawn',
    e359.ok && s359.ok && s359.value.dayNightPhase === 'dawn' && s359.value.fovRadius > 5 && s359.value.fovRadius < 9,
    s359.ok ? `fov=${s359.value.fovRadius.toFixed(3)} phase=${s359.value.dayNightPhase}` : s359.error)
  const e1199 = await evl(page, () => window.__debug.setState({ timeOfDay: 1199 }))
  const s1199 = await evl(page, () => window.__debug.getState())
  test('B10 timeOfDay 1199 (last dusk minute): phase dusk, fov > 5 (not yet night)',
    e1199.ok && s1199.ok && s1199.value.dayNightPhase === 'dusk' && s1199.value.fovRadius > 5,
    s1199.ok ? `fov=${s1199.value.fovRadius.toFixed(3)} phase=${s1199.value.dayNightPhase}` : s1199.error)
...
section('D. Dispose/re-init cycles (setSeed loop) — leak/orphan check')
{
  ...
  const sceneCount = () => evl(page, () => {
    const g = window.__debug
    // The harness doesn't expose the scene; count chunk groups via the
    // registry cross-check + a marker of what the scene SHOULD contain:
    // player mesh + 4 lights + solid chunk groups + origin marker.
    let withGroup = 0
    let orphaned = 0
    const wm = window.__debug.world
    // Reach the live WorldManager through the debug graph? Not exposed — use
    // chunkInfo across the loaded band instead: every tracked solid chunk
    // must have a composer and a group that are consistent.
    for (let dy = -3; dy <= 3; dy++) for (let dx = -3; dx <= 3; dx++) {
      const c = wm.chunkInfo(dx, dy)
      if (c.tracked && c.tileCount > 0) withGroup++
    }
    return { withGroup, chunks: wm.loadedChunkCount }
  })
  ...
  test('D1 10 regenerate cycles: loaded chunk count stable (no orphaned/leaked chunk stubs)',
    before.ok && after.ok && after.value.chunks === before.value.chunks,
    before.ok && after.ok ? `${before.value.chunks} → ${after.value.chunks}` : (before.error || after.error))
  ...
  await waitChunksDrained(page, 20000)
  const afterSpam = await sceneCount()
  const st = await evl(page, () => window.__debug.getState())
  test('D2 12 rapid regenerates (no drain between): settles, no exception, count stable',
    spam.ok && afterSpam.ok && st.ok && afterSpam.value.chunks === after.value.chunks && st.value.loadedChunkCount === after.value.chunks,
    afterSpam.ok && st.ok ? `chunks=${afterSpam.value.chunks} state=${st.value.loadedChunkCount}` : (afterSpam.error || st.error))
...
section('F. Night→dawn tween while MOVING (R5 at speed)')
{
  ...
  // Park the clock deep in dusk, then hold W THROUGH the natural boundary
  // crossing (fov 5, chunk radius 2→1) — chunk churn + dims refresh while the
  // player streams through new chunks.
  await evl(page, () => window.__debug.setState({ timeOfDay: 1100 }))
  await waitChunksDrained(page)
  await page.keyboard.down('KeyW')
  const crossed = await waitFor(page, () => window.__debug.world.fovRadius <= 5.0, 30000)
  await page.keyboard.up('KeyW')
  await waitChunksDrained(page, 20000)
...
// ─────────────────────────────────────────────────────────────
section('G. Window blur / visibilitychange: keys must not stick (known gap)')
{
  const page = await newPage()
  await loadDebug(page)
  await evl(page, () => window.__debug.gotoFixture('slice-c-demo'))
  ...
  // Behavior: hold W, then "tab out" (blur + visibilitychange — the OS
  // swallows the keyup in real life). The game has NO blur handler, so the
  // key stays down and the cube keeps walking when focus returns.
  await page.keyboard.down('KeyW')
  await new Promise(res => setTimeout(res, 150))
  await page.evaluate(() => {
  ...
  const movedDuring = during.ok && after.ok && after.value.y > during.value.y + 0.5
  test('G1 KNOWN GAP (repro): key held across blur+visibilitychange keeps walking (no key reset)',
    movedDuring === true,
    during.ok && after.ok ? `y ${during.value.y.toFixed(2)} → ${after.value.y.toFixed(2)} (kept moving across blur)` : (during.error || after.error))
...
section('H. Click-to-move arrival + target switch mid-move')
{
  ...
  // Click (4,4), then mid-flight click (9,1): the player must arrive at the
  // SECOND target (last click wins), and must STOP there.
  const p1 = await evl(page, () => window.__debug.world.projectTile(4, 4))
  const p2 = await evl(page, () => window.__debug.world.projectTile(9, 1))
  if (p1.ok && p1.value && p2.ok && p2.value) {
    ...
    await new Promise(res => setTimeout(res, 150)) // mid-flight to (4,4)
    await evl(page, (cx, cy) => document.dispatchEvent(new MouseEvent('click', { clientX: cx, clientY: cy, bubbles: true })), p2.value.x, p2.value.y)
    const arrived = await waitFor(page, () => {
      const p = window.__debug.world.player
      return Math.abs(p.x - 9.5) < 0.3 && Math.abs(p.y - 1.5) < 0.3
    }, 8000, 100)
    await new Promise(res => setTimeout(res, 300)) // ensure it stops
    const pos = await evl(page, () => window.__debug.world.player)
    const v = pos.ok ? pos.value : null
    test('H1 mid-flight click switches target: player arrives at the SECOND tile center (9,1)',
      arrived !== null && pos.ok && v && Math.abs(v.x - 9.5) < 0.2 && Math.abs(v.y - 1.5) < 0.2,
      pos.ok ? `player=(${v.x.toFixed(2)}, ${v.y.toFixed(2)}) target=(9.5,1.5)` : pos.error)
    test('H2 player STOPS at the arrival point (moveTarget cleared, no overshoot drift)',
      pos.ok && v && (Math.abs(v.x - 9.5) < 0.2 && Math.abs(v.y - 1.5) < 0.2),
      pos.ok ? `player=(${v.x.toFixed(2)}, ${v.y.toFixed(2)})` : pos.error)
  } else {
    test('H1 mid-flight click switch — SKIPPED (tiles not projectable)', false, p1.error || p2.error || '')
  }
