...
/** Completion round: slice-c-demo leaves the debug overlay OPEN with the
 *  command input FOCUSED, and the InputManager e.target guard (input/
 *  textarea/#debug-overlay) now correctly swallows WASD while typing — so
 *  probes that need REAL movement must move focus off the overlay first
 *  (simulates the dev clicking the canvas). Guard-specific probes (P8j in
 *  edge-probes, G1 here) deliberately do NOT blur before their keys. */
async function blurOverlayFocus(page) {
  await evl(page, () => {
    const el = document.activeElement
    if (el instanceof HTMLElement && el.closest('#debug-overlay')) el.blur()
  })
}
...
section('A. Void walk + far teleports (falling off the world)')
{
  ...
  // Completion round: the fixture's overlay input holds focus — WASD must
  // be tested with focus elsewhere (typing into the overlay is guarded).
  await blurOverlayFocus(page)
...
section('C. Rapid WASD ↔ click-to-move interleaving')
{
  ...
  await evl(page, () => window.__debug.gotoFixture('slice-c-demo'))
  await waitChunksDrained(page)
  // Completion round: fixture overlay input holds focus — blur it so WASD
  // drives the game (typing into the overlay is guarded).
  await blurOverlayFocus(page)
...
section('F. Night→dawn tween while MOVING (R5 at speed)')
{
  ...
  await evl(page, () => window.__debug.gotoFixture('slice-c-demo'))
  await waitChunksDrained(page)
  // Completion round: fixture overlay input holds focus — blur it so the
  // held W drives the game (typing into the overlay is guarded).
  await blurOverlayFocus(page)
...
section('G. Window blur / visibilitychange: keys must reset (completion round)')
{
  ...
  // the tab. The fixture's overlay input holds focus, so blur it FIRST —
  // otherwise the e.target guard swallows the W and the test proves nothing.
  await blurOverlayFocus(page)
  await page.keyboard.down('KeyW')
  await new Promise(res => setTimeout(res, 150))
  await page.evaluate(() => {
