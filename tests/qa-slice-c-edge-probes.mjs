...
section('P8. Debug overlay commands (real keyboard input)')
{
  ...
  // 0,0) — the earlier tp 3 -2 is intentionally gone. Typing 'bogus-command'
  // pressed 's' + 'a' + 'd' (see P8j) so the player is only NEAR spawn.
...
// ─────────────────────────────────────────────────────────────
section('P8j. Known Minor: WASD typed into the overlay input leaks into movement')
{
  // CONFIRMED FINDING: InputManager's window keydown handler never checks
  // e.target, so letter keys typed into #debug-overlay-input bubble to the
  // window and set keys['KeyW'|'KeyA'|'KeyS'|'KeyD']. A dev typing a command
  // containing those letters nudges the cube. Dev-only surface, so Minor.
  const page = await newPage()
  await loadDebug(page)
  await evl(page, () => window.__debug.gotoFixture('slice-c-demo'))
  await waitChunksDrained(page)
  ...
  await new Promise(res => setTimeout(res, 600))
  const p1 = await evl(page, () => window.__debug.world.player)
  test('P8j holding "w" while the overlay input is focused moves the player (KNOWN MINOR: WASD leaks through the input)',
    p0.ok && p1.ok && p1.value.y - p0.value.y > 0.05,
...
// ─────────────────────────────────────────────────────────────
section('P9. Natural-tween FOV dims freeze between chunk-radius crossings (confirmed Major)')
{
  // WHY THIS PROBE EXISTS: _updateRadii only reapplies dims when the CHUNK-
  // SPACE load radius changes (ceil((fov+3)/8)). Between crossings — for the
  // whole dusk (fov 8.99→5.01, ~120 in-game min) and dawn (5.01→8.99, ~60 min)
  // — the natural per-frame tween leaves every retained tile's dim frozen at
  // the last crossing's values. The harness setState/fastForward paths CANNOT
  // show this (wm.setTimeOfDay calls _applyFovDims unconditionally), so this
  // probe parks the clock via the harness and then lets the NATURAL clock
  // advance through the band. Confirmed locally: tile (6,6) at tod 1122 reads
  // dim 1.0 vs fog 0.789 (dusk) and dim 0.06 vs fog 0.255 (dawn).
  const page = await newPage()
  await loadDebug(page)
  await evl(page, () => window.__debug.gotoFixture('slice-c-demo'))
  await waitChunksDrained(page)
  ...
  // loadChunkR stays 2 — and read the retained tile.
  ...
  const staleDusk = v && v.dim !== null && s && v.dim > v.fog + 0.2
  test('P9a CONFIRMED MAJOR: mid-dusk natural tween keeps retained tiles at the dusk-START dims (dim > fog + 0.2)',
    crossedMidDusk !== null && dusk.ok && staleDusk,
  ...
  // Dawn: park just after dawn starts (fov ~5.33), advance NATURALLY to fov
  // ~7.0 (tod ~330) — dims must still be frozen at the night values.
  ...
  const staleDawn = dv && dv.dim !== null && Math.abs(dv.dim - dv.fog) > 0.2
  test('P9b CONFIRMED MAJOR: mid-dawn natural tween keeps retained tiles at the NIGHT dims (dim ≠ fog in the band)',
    crossedMidDawn !== null && dawn.ok && staleDawn,
