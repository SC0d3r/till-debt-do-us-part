...
//   T11 zero page/console errors across the whole battery
//
...
section('T7. Real input works after a preview cycle (composer hover)')
{
  ...
  // helper and dispatch a genuine pointermove event.
  ...
  test('T7a real pointermove highlights a tile after preview→showcase cycle', r.ok && r.value?.x === 4 && r.value?.y === 4 && r.value?.variant === 'grass-plain',
